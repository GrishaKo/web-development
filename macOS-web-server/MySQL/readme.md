...
> 
> `#log_bin` - укажем имя бинарного лога, но включать не будем, закомментируем это параметр, потому что при его включении MySQL сервер буде работать медленнее, а необходим лог впервую очередь при репликации и полезен при восстановлении после сбоев (подробнее в документации [MySQL](https://dev.mysql.com/doc/refman/5.5/en/binary-log.html) и [MariaDB](https://mariadb.com/kb/en/library/overview-of-the-binary-log/)).
> 
